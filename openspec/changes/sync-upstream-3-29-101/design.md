...
3. **~~Locale data merge is additive, not overwrite~~ — revised after implementation.** This decision assumed `items.ndjson`/`stats.ndjson` were fork-translated content requiring manual merge. Investigation during implementation found they are official Path of Exile game-data extracts (icon URLs confirm `cmn-Hant` already points at `webtw.poecdn.com`, the official Taiwan CDN, natively in upstream) — upstream already ships complete, official Traditional Chinese translation for all new content. These files (all locales, including `cmn-Hant`) were adopted wholesale, no manual translation needed. `app_i18n.json` (UI copy) genuinely is hand-curated and richer than upstream's stock file (338 vs upstream's minimal keys), but this sync introduced zero new upstream keys (`en/app_i18n.json` is byte-identical fork vs upstream), so it required no merge either — left untouched. `patrons.json` is data-only (no TW customization) — adopted wholesale.
