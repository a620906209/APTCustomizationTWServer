...
3. **Locale data merge is additive, not overwrite — and new content is translated, not stubbed.** For `cmn-Hant/app_i18n.json`, `items.ndjson`, `stats.ndjson`: diff upstream's key set against the fork's, and for every key upstream introduced that doesn't yet exist in `cmn-Hant`, produce a real Traditional Chinese translation before merging it in (using the upstream `en` value as the source text to translate from, not as a left-in placeholder). No English fallback strings should remain in `cmn-Hant` data after this change. Never let an upstream key overwrite an existing TW translation for a key that already exists in `cmn-Hant`. `en`/`ko`/`ru` ndjson files are not TW-customized — adopt wholesale. `patrons.json` is data-only (no TW customization) — adopt wholesale.
